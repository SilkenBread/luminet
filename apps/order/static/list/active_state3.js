// ============================================================
// Order Active State 3 - Validación
// ============================================================
...
function renderTable(data) {
    ...
    data.forEach(ot => {
        // Determine action buttons based on reassign status
        let actionButtons = '';

        if (ot.reassign) {
            // Has reassign cause — show "Accept Reassign" button
            actionButtons = `
                ...
                <button onclick="acceptReassign(${ot.id})"
                        ...
                        title="Aceptar reasignación">
                    <i data-lucide="check-circle" class="w-4 h-4"></i>
                </button>
                <button onclick="rejectOrder(${ot.id})"
            ...
            // Normal — show "Close" and "Reject" buttons
            actionButtons = `
                <button onclick="locateOrder(${ot.fk_pqr__id})"
                        class="inline-flex items-center p-2 text-green-600 hover:text-green-800 hover:bg-green-50 rounded-lg transition-colors duration-200"
                        title="Ubicar en mapa">
                ...
                <button onclick="closeOrder(${ot.id})"
                        class="inline-flex items-center p-2 text-emerald-600 hover:text-emerald-800 hover:bg-emerald-50 rounded-lg transition-colors duration-200"
                        title="Cerrar orden">
        ...
        const reassignBadge = ot.reassign
            ? `<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">${ot.reassign}</span>`
...
function populateReviewModal(pqr) {
    ...
    if (pqr.route_history && pqr.route_history.length > 0) {
        pqr.route_history.forEach((r, i) => {
            routeBody.append(`<tr class="hover:bg-gray-50"><td class="px-4 py-2 text-sm text-gray-700">${i + 1}</td><td class="px-4 py-2 text-sm text-gray-700">${r.state || '-'}</td><td class="px-4 py-2 text-sm text-gray-700">${r.date_entry || '-'}</td><td class="px-4 py-2 text-sm text-gray-700">${r.date_exit || '-'}</td></tr>`);
        });
    } else {
...
function closeOrder(otId) {
    Swal.fire({
        title: '¿Cerrar Orden?',
        text: `¿Deseas cerrar la OT #${otId}?`,
        icon: 'question',
        ...
        if (result.isConfirmed) {
            orderStatusChange(otId, 4);
        }
    });
}

function rejectOrder(otId) {
    Swal.fire({
        title: '¿Rechazar Orden?',
        text: `¿Deseas rechazar la OT #${otId} y devolverla a asignación?`,
        icon: 'warning',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            orderStatusChange(otId, 1);
        }
    });
}

function acceptReassign(otId) {
    Swal.fire({
        title: '¿Aceptar Reasignación?',
        text: `¿Deseas aceptar la reasignación de la OT #${otId}?`,
        icon: 'question',
        showCancelButton: true,
